...
If they instead use their power from minute 2 to X+1, we only have to use data from minutes 1, 2, X and X+1 to update our answer A.
 */
public class GrumpyBookstoreOwner_1052 {
    public int maxSatisfied(int[] customers, int[] grumpy, int minutes) {
        var n = customers.length;
        var unSatisfiedSum = 0;
        var satisfiedSum = 0;
        ...
        var max = unSatisfiedSum;
        for (var i = minutes-1; i < n; i++) {
            if (grumpy[i] == 1) {
                unSatisfiedSum += customers[i];
            } else {
                satisfiedSum += customers[i];
            ...
            max = Math.max(max, unSatisfiedSum);
            unSatisfiedSum -= customers[i+1 - minutes] * grumpy[i+1 - minutes];
        }

        return satisfiedSum + Math.max(max, unSatisfiedSum);
    }
}
