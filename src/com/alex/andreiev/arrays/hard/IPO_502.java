...
public class IPO_502 {
    ...
    public int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        var projectsHeap = new PriorityQueue<AbstractMap.SimpleEntry<Integer, Integer>>(
                Comparator.comparingInt(AbstractMap.SimpleEntry::getKey)
        );

        for (int i = 0; i < profits.length; i++) {
            projectsHeap.offer(new AbstractMap.SimpleEntry<>(capital[i], profits[i]));
        }

        var maxHeap = new PriorityQueue<Integer>(Comparator.reverseOrder());

        while (k > 0) {
            while (!projectsHeap.isEmpty() && projectsHeap.peek().getKey() <= w) {
                maxHeap.offer(projectsHeap.poll().getValue());
            }
